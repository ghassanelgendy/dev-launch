...
Developer
  ...
  -> Docker Build
  -> Amazon ECR
  -> DevLaunch CLI (validate + generate)
  -> GitOps Repo/Folder Update
  -> Flux CD
  -> AWS EKS
...
> I built DevLaunch, an Internal Developer Platform Lite on AWS EKS that lets developers deploy services from a simple YAML config. It validates config, builds and pushes Docker images via GitLab CI/CD, generates Helm values, and uses Flux CD for GitOps deployments, with ingress, monitoring, and rollback workflows.

## CV Bullet

> Built a GitOps-based internal developer platform on AWS EKS enabling deployment from simple YAML service definitions; automated Docker builds, ECR pushes, Helm values generation, Flux CD delivery, ingress routing, monitoring dashboards, and rollback workflows.
